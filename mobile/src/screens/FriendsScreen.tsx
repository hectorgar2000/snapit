...
export default function FriendsScreen({ session }: Props) {
  ...
  if (session.isGuest) return (
    <View style={styles.guestState}>
      <Text style={styles.guestTitle}>👥 Gestión de amigos</Text>
      <Text style={[S.muted, { textAlign: 'center', marginBottom: 20 }]}>
        Regístrate para añadir amigos y comparar resultados cada día
      </Text>
    </View>
  );
  ...
  async function loadFriends() {
    ...
    try {
      ...
      Alert.alert('Error', 'No se pudo cargar la lista de amigos.');
    } finally {
      setLoading(false);
    }
  }
  ...
  function onSearchChange(text: string) {
    ...
    searchTimer.current = setTimeout(async () => {
      setSearching(true);
      try {
        const results = await apiSearchUsers(text, session.token);
  ...
  async function acceptFriend(username: string) {
    try {
      await apiAcceptFriend(username, session.token);
      loadFriends();
    } catch (e: any) {
      Alert.alert('Error', e.message);
    }
  ...
  async function removeFriend(username: string) {
    Alert.alert('Eliminar amigo', `¿Eliminar a @${username}?`, [
  ...
  const ActionBtn = ({ user }: { user: FriendInfo }) => {
    if (user.status === 'accepted')       return <View style={styles.btnPending}><Text style={styles.btnPendingText}>✓ Amigos</Text></View>;
    if (user.status === 'pending_sent')   return <View style={styles.btnPending}><Text style={styles.btnPendingText}>⏳</Text></View>;
    if (user.status === 'pending_received') return (
      <TouchableOpacity style={styles.btnAccept} onPress={() => acceptFriend(user.username)}>
        <Text style={{ color: C.green, fontWeight: '700' }}>✓ Aceptar</Text>
      </TouchableOpacity>
    );
    return (
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListHeaderComponent={received.length > 0 ? (
            <Text style={S.sectionTitle}>Solicitudes recibidas</Text>
          ) : null}
          ListEmptyComponent={
              ...
              return (
                <View style={styles.friendRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={S.muted}>@{r.username} · quiere ser tu amigo</Text>
                  </View>
            ...
            return (
              <View style={styles.friendRow}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={S.muted}>@{f.username} · 🔥{f.current_streak} · {f.total_score.toLocaleString()} pts</Text>
                </View>
